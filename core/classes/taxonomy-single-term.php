...
class SetRobot_Taxonomy_Single_Term
{
    ...
    public function input_element()
    {
        ...
        ?>
        <div id="taxonomy-<?= $this->slug;?>" class="<?= $class;?>">
            <?php
                $this->{$el_open_cb}();
                $this->term_fields_list();
                $this->{$el_close_cb}();
    ...
    {
        ?>
        <select style="display:block;width:100%;margin-top:12px;" name="<?= $this->namefield;?>" id="<?= $this->slug;?>checklist" class="form-no-clear">
            <?php if (! $this->force_selection) : ?>
                <option value="0">
                    <?php
                        echo esc_html(apply_filters('taxonomy_single_term_select_none', __('None')));
                    ?>
                </option>
            <?php endif;
    }
    ...
    {
        ?>
        <ul id="<?= $this->slug;?>checklist" data-wp-lists="list:<?= $this->slug; ?>" class="categorychecklist form-no-clear">
            <?php if (! $this->force_selection) : ?>
                <li style="display:none;">
                    <input id="taxonomy-<?= $this->slug;?>-clear" type="radio" name="<?= $this->namefield;?>" value="0" />
                </li>
            <?php endif;
    }
    /**
    ...
    public function radio_close()
    {
        ...
        <p style="margin-bottom:0;float:left;width:50%;">
            <a class="button" id="taxonomy-<?= $this->slug;
        ?>-trigger-clear" href="#"><?php _e('Clear');
        ?></a>
        </p>
        <script type="text/javascript">
            jQuery(document).ready(function($){
                $('#taxonomy-<?= $this->slug;?>-trigger-clear').click(function(){
                    $('#taxonomy-<?= $this->slug;?> input:checked').prop( 'checked', false );
                    $('#taxonomy-<?= $this->slug;?>-clear').prop( 'checked', true );
                    return false;
                });
            });
        </script>
    ...
    public function terms_adder_button()
    {
        ...
        <p style="margin-bottom:0;float:right;width:50%;text-align:right;">
            <a class="button-secondary" id="taxonomy-<?= $this->slug;
        ?>-new" href="#"<?php if ('radio' == $this->input_element) : ?> style="display:inline-block;margin-top:0.4em;"<?php endif;
        ?>><?php _e('Novo');
        ?></a>
        </p>
        <script type="text/javascript">
            jQuery(document).ready(function($){
                $('#taxonomy-<?= $this->slug;?>-new').click(function(e){
                    e.preventDefault();
                    var termName = prompt( "Adicionar novo(a) <?= esc_attr($this->taxonomy()->labels->singular_name);?>", "Novo(a) <?= esc_attr($this->taxonomy()->labels->singular_name);?>" );
                    if( ! termName ) {
                        return;
                    }
                    if(termName != null) {
                        var data = {
                            ...
                            'term_name' : termName,
                            'taxonomy'  : '<?= $this->slug;?>',
                            'nonce'     : '<?= wp_create_nonce('taxonomy_'. $this->slug, '_add_term') ;?>'
                        };
                        $.post( ajaxurl, data, function(response) {
                            ...
                            if( response.success ){
                                ...
                                ?>
                                    $('#taxonomy-<?= $this->slug;?> option').prop( 'selected', false );
                                <?php
                                    endif;
                                ?>
                                $('#<?= $this->slug;?>checklist').append( response.data );
                            } else {
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function($){
                ...
                var changeToRadio = function( $context ) {
                    ...
                    var $taxListInputs = $context.find( '.<?= $this->slug;?>-checklist li input' );
                ...
                $postsFilter
                    ...
                    .on( 'change', '.transformed-to-radio', function() {
                        var $this = $(this);
                        $siblings = $this.parents( '.<?= $this->slug;?>-checklist' ).find( 'li .transformed-to-radio' ).prop( 'checked', false );
    ...
    public function walker()
    {
        ...
        $this->walker = new SetRobot_Taxonomy_Single_Term_Walker($this->taxonomy()->hierarchical, $this->input_element);
        return $this->walker;
    }
    /**
